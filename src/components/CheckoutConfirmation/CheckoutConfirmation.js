...
class CheckoutConfirmation extends Component {
	...
	handleClick = () => {
		const {
			purchasedItems,
			checkout: { deliveryAddress, creditCard, paymentMethod },
			history,
			setLoading
		} = this.props;
		setLoading(true);
		...
		if (paymentMethod === CREDIT_CARD) {
			...
			postCreditCard(data)
				.then(res => {
					if (res.status === 201) {
						history.push(
							`/payment-response?cartId=${res.data.cartId}`
						);
			...
			postWireTransfer(data).then(res => {
			...
			<Fragment>
				...
				>
					...
					<div className='footer-delivery justify-content-between row'>
						<p>
							{translate('checkout.payment.cash.placeOrder')}{' '}
							<span>
								{' '}
								{translate('checkout.payment.cash.terms')}{' '}
							</span>
						</p>
						<button
							type='button'
						...
						>
							<div>
								<p>
									{translate('checkout.payment.cash.total')}
								</p>
								<p>
									{this.props.grandTotal.toFixed(2)}
									<sub>{translate('general.currency')}</sub>
								</p>
							</div>
							<span>
								{translate('checkout.confirm.placeOrder')}{' '}
								<i
									className={`icon-arrow-${right(direction)}`}
								/>
							</span>
						</button>
					</div>
