...
class Quotations extends Component {
	...
	componentDidMount() {
		...
		this.props.getClosedRequests(this.props.customer.id)
			.then(() => {
				this.resetLoading();
			});
		let query = queryString.parse(this.props.location.search.slice(1));
	...
	render() {
		...
		} else if (_.isEmpty(quotations.pending) && _.isEmpty(quotations.completed) && _.isEmpty(quotations.closed)) {
			...
			renderQuotation = <Fragment>
				<Nav tabs>
					...
					<NavItem>
						<NavLink
							...
							{translate("quotationRequest.closed")}
						</NavLink>
					</NavItem>
				</Nav>
				<TabContent activeTab={this.state.activeTab}>
					...
					<TabPane tabId="closed">
						<ul className="list-unstyled" id="closed-list">
							{
								quotations.closed.map(closed => {
									return <PendingRequest
										key={closed.id}
										list={closed}
										translate={translate}
