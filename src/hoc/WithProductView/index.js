...
const WithProductView = WrappedComponent => {
  return class extends Component {
    ...
    removeSelectedUrl = itemToRemove => {
      const newParam = this.state.params.find(
        param => param.checkId === itemToRemove.id
      );
    ...
    renderSearch = (
      ...
      console.log(isVehicleSelected, selectedVehicle);

      return filtration.options.map((option, index) => {
		  console.log(option)
        const key =
          currentLanguage === constant.EN ? "filterTitle" : "filterTitleAr";
        ...
        return (
          <ul key={index} className="options-list">
            <li>
              {isVehicleSelected ? (
                <div className="checkbox">
                  <input
                    ...
                    value={checkLabel}
                    checked={selectedVehicle.name === value}
                  />
